...
static struct gt9147_dev gt9147;
...
static int gt9147_read_regs(struct gt9147_dev *dev, u16 reg, u8 *buf, int len)
{
	...
	/* msg[0]为发送要读取的首地址 */
	msg[0].addr = client->addr;			/* gt9147地址 */
	msg[0].flags = !I2C_M_RD;			/* 标记为发送数据 */
	msg[0].buf = &regdata[0];			/* 读取的首地址 */
	msg[0].len = 2;						/* reg长度*/

	/* msg[1]读取数据 */
	msg[1].addr = client->addr;			/* gt9147地址 */
...
/**************************************************************************************/
  ...
  * @return          : 0，成功;其他负值,失败
  */
static int gt9147_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	u8 data, ret;
 ...
 * @return          : 0，成功;其他负值,失败
 */
static int gt9147_ts_remove(struct i2c_client *client)
{
...
/* 传统驱动匹配表 */ 
static const struct i2c_device_id gt9147_ts_id[] = {
	{ "goodix,gt9147", 0, },
	{/* Sentinel */}
};
...
/* i2c驱动结构体 */	
static struct i2c_driver gt9147_ts_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.of_match_table = gt9147_of_match,
		.name = "gt9147",
	},
	.probe = gt9147_ts_probe,
	.remove = gt9147_ts_remove,
	.id_table = gt9147_ts_id,
};
...
static int __init gt9147_init(void)
{
    ...
    /* 注册i2c_driver */
    ret = i2c_add_driver(&gt9147_ts_driver);
...
{
    /* 卸载i2c_dirver */
    i2c_del_driver(&gt9147_ts_driver);
}
